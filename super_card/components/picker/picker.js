...
export default {
	...
	init(id, opts = {}) {
		...
		const component = new Component({
          ...
          if(id == 'bglist') this.render(options.value)
          
				},

        cancel() {
            const cols = this.getComponentData().cols
            const params = this.updateValue(cols, options.value)
            typeof options.onCancel === `function` && options.onCancel(params)
            this.hide()
          ...
          const cols = this.getComponentData().cols
          const params = this.updateValue(cols, that.temp[id])

          var temp = typeof options.onDone === `function` ? options.onDone(params) : false
          
          if(temp !== false) this.hide()

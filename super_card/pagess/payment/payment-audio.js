...
Page({
  ...
  confirmPay: function (e){
    ...
    app.util.request({
      'url': 'entry/wxapp/uploadAudioPay',
      data: {
  ...
  successPayAfter: function () {
    ...
    setTimeout(function () {
      ...
      wx.redirectTo({
        url: '../../pages/sound-recording/sound-recording?card_id=' + that.data.card_id
      })
  ...
  onLoad: function (options) {
    ...
    if(typeof options.umoney == 'undefined' || typeof options.card_id == 'undefined'){
    ...
    this.setData({ card_id: options.card_id , uMoney: uMoney, account_intro: app.config.getConf('account_intro') })
  ...
  showMoneyPay: function (isCheck){
    ...
    var fee = Math.round(app.config.getConf('audio_upload_fee') * 100) / 100

...
Page({
  ...
  successPayAfter: function () {
    ...
    var that = this
    for (var x in that.data.prevPage.data.styles)
      if (that.data.prevPage.data.styles[x].id == that.data.choiceCardStyle)
        that.data.prevPage.data.styles[x].buy = 1

    that.data.prevPage.setData({ styles: that.data.prevPage.data.styles })
  ...
  onLoad: function (options) {
    ...
    if (typeof options.umoney == 'undefined' || typeof options.choiceCardStyle == 'undefined' || options.price == 'undefined'){
      wx.navigateBack({ delta: 1 })
      return
    }
    var cardStyle = app.config.getConf('card_style')
    //for(var x in vipSet)
    var style_name = cardStyle[parseInt(options.choiceCardStyle) - 1].name

    var uMoney = parseFloat(options.umoney)

    this.setData({ uMoney: uMoney, style_name: style_name, choiceCardStyle: options.choiceCardStyle, price: parseFloat(options.price), account_intro: app.config.getConf('account_intro') })

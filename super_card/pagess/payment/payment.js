// super_card/pages/payment/payment.js\
var app = getApp()
Page({
  ...
  showMoneyPay: function (isCheck){
    ...
    var fee = Math.round(app.config.getConf('new_card_fee') * 100) / 100
    fee = parseFloat(fee.toFixed(2))

    var new_card_percent= app.config.getConf('new_card_percent')
    if (new_card_percent > 0){   
      var free_card_num = app.config.getConf('free_card_num')
      if(this.data.cardNum > free_card_num){
        fee += (this.data.cardNum - free_card_num) * (new_card_percent / 100)
      }
    }
    fee = parseFloat(fee.toFixed(2))

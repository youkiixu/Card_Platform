  ...
  if (typeof obj !== "object") {
    ...
    for (var prop in obj) {
      if (obj[prop].constructor === RegExp || obj[prop].constructor === Date) {
        ...
        //递归
        newObj[prop] = util.deepCopy(obj[prop]);
      } else {
        newObj[prop] = obj[prop];
      }
    }
...
*/
util.url = function (action, querystring, needSess = true) {
	var app = getApp();
...
util.footer = function ($this, card_id) {
  ...
  console.log()
	...
	let tabBar = util.deepCopy(app.tabBar);
  
  
  //console.log(tabBar)

  var list = []
  var have_bar = app.overtHaveBar
  var backSet = app.config.getConf('custom_bottom_nav')

  
  if (!backSet || backSet.length != 4) backSet = false
  
  //console.log(have_bar)
  for(var x in have_bar){

    if(have_bar[x] == 1){
      list.push(tabBar.list[x])
    }else{
  ...
  //console.log(card_id)
    ...
    if (backSet !== false && typeof backSet[i].iconPath != 'undefined'){
      tabBar['list'][i]['iconPath'] = backSet[i].iconPath
      tabBar['list'][i]['selectedIconPath'] = backSet[i].selectedIconPath
    }
    tabBar['list'][i]['pageUrl'] = tabBar['list'][i]['pagePath'].replace(/(\?|#)[^"]*/g, '')
    tabBar['list'][i]['pagePath'] = tabBar['list'][i]['pageUrl'] + '?card_id=' + card_id
  ...
  var selectedColor = app.config.getConf('custom_bottom_color')
  if (selectedColor) tabBar.selectedColor = selectedColor
...
util.message = function(title, redirect, type) {
  ...
  var confirmText = false
    ...
    confirmText = typeof title.confirmText != 'undefined' ? title.confirmText : false
	...
	if (type == 'success') {
      ...
      confirmColor: '#54b536',
      cancelColor: '#54b536',
      confirmText: (confirmText === false ? '我知道了' : confirmText),
			complete : function() {
				if (url) {
					wx[redirectFunction]({
						url: url,
